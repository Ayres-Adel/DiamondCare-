...
<head>
    ...
    <link rel="stylesheet" href="admin_descending.css">
...
$count = 0;
  ...
  if(mysqli_num_rows($result)>0)
    ...
    $sql = "SELECT *FROM service ORDER BY date_appointment DESC";
    ...
    if(mysqli_num_rows($result)>0)
    {
        ...
        echo '<div class="text"><center>Descending order of <span class="BOOK">patients</span></center></div>';
        while ($row = mysqli_fetch_assoc($result)) {
            $date = $row["date_appointment"];
            $Matricule = $row["Matricule"];
            echo '<div class="flex_box">';
            echo '<div class="Matricule">Matricule: ' . $Matricule . '</div>';
            echo '<div class="date_appointment">date_appointment: ' . $date . '</div>';
            echo '</div>';  
        }
        echo '</div>';  
    }
